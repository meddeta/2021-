...
/**
 * The purpose of this class is to provide a converter from an array of ints to an equivalent
 * string representation, and a shorthand for writing and testing invariants in any program.
 * 
 * @author andy
 * 
 */
public class TestHelper {
	 ...
	 * 
	 * @return a String representation of input int array ints[0..n-1] 
	 *        formatted as   "[ ints[0] , ints[1] , ... , ints[n-1] ]"
	 *        in O(n) time.
	 * 
	 * @param ints
	 *            the input array.
	...
	public static String stringInts(int[] ints) {
		...
		/*
		 * To ensure O(n) time, we can use StringBuilder with repeated appends. If we use String
		 * with repeated concatenations, it will take O(n^2) time, and it will wastefully create
		 * O(n) distinct intermediate strings. This is because String is immutable. But the mutable
		 * StringBuilder is internally implemented as a dynamically expandable array, representing a
		 * character sequence. See the Java API. We prefer StringBuilder over the older version
		 * called StringBuffer.
		 */
